...
router.get("/:id", (req, res) => {
  ...
  connection.query(
    "SELECT id, title, DAY(start_date) AS start_day,MONTH(start_date) AS start_month, YEAR(start_date) AS start_year,DAY(end_date) AS end_day, MONTH(end_date) AS end_month, YEAR(end_date) AS end_year, days_left FROM prescriptions WHERE user_id = ?",
    [id],
    (err, results) => {
      if (err) {
        res.status(500).json({
...
router.post("/", (req, res) => {
  ...
  return connection.query(
    ...
    (err, results) => {
      ...
      let sql =
        "INSERT INTO `drugs` (id, prescription_id, name, duration, times_a_day, dose, notes, doses_taken, days_left, doses_supposed) VALUES ";
      drugsArr.map((drug) => {
      ...
      return connection.query(sql, (err2, records) => {
        if (err2) {
          ...
          res.status(200).send("The prescription was created");
        }
